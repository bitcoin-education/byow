...
class ReceiveBitcoinTest extends GuiTest {
    ...
    private ExtendedPubkeyService extendedPubkeyService

    private SegwitAddressGenerator segwitAddressGenerator

    def setup() {
        segwitAddressGenerator = new SegwitAddressGenerator(new AddressPrefixFactory(AddressPrefixFactory.REGTEST))
    ...
    def "should receive bitcoin"() {
        ...
        then:
            ...
            addressIsValid(address, 0, mnemonicSeed)
    }
    ...
    def "should receive bitcoins in two transactions to different addresses"() {
        ...
        then:
            ...
            addressIsValid(address, 0, mnemonicSeed)
            addressIsValid(secondAddress, 1, mnemonicSeed)
    }
    ...
    private void sendBitcoinAndWait(String address, String expectedTotalAmount = "1.0", int expectedTotalSize = 1) {
        ...
        waitFor(TIMEOUT, SECONDS, {
            TableView<AddressRow> tableView = lookup("#addressesTable").queryAs(TableView)
    ...
    boolean addressIsValid(String address, Integer index, String mnemonicSeedString) {
        MnemonicSeed mnemonicSeed = new MnemonicSeed(mnemonicSeedString)
        ExtendedPrivateKey masterKey = mnemonicSeed.toMasterKey("", MAINNET_PREFIX.getPrivatePrefix())
