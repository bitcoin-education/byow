...
public class NodeReceivedByAddressClient {
    ...
    public List<NodeAddress> listAddresses(String walletName, int minConf, boolean includeEmpty, boolean includeWatchOnly) {
        NodeAddress[] nodeAddresses = nodeClient.makeRequest("listreceivedbyaddress", new ParameterizedTypeReference<>(){}, "wallet/".concat(walletName), minConf, includeEmpty, includeWatchOnly);
        return Arrays.stream(nodeAddresses).toList();
    }
}
